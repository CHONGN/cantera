                   ...
                   integer* diagg, doublereal rdt)
{
    ...
    if (m_ilr == LeftInlet) {
        ...
        if (!StFlow::getTwoPointControlEnabled()){
             //to assign mdot, which is from oxider inlet, to mass flow of oxider inlet at the rightside boundary
             rb[c_offset_uo]+=0.0;
             rb[c_offset_L]+= 0.0;
             rb[c_offset_U] += m_mdot; // u
        }
	if (StFlow::getTwoPointControlEnabled()){
             //to assign the simulation result of uo to mass flow of oxider inlet at the rightside boundary
             m_mdot = m_flow->density(last_point_ndx)*fabs(xb[c_offset_uo]);
             rb[c_offset_L]+= 0.0; 
             rb[c_offset_U] += m_mdot; // u
        }
	//rb[c_offset_U] += m_mdot; // u
